...
const parser = new Parser({
  timeout: 10000,
  headers: { "User-Agent": "ICE-Tracker/2.0 (Transparency)" },
});
...
/** In-memory cache (v2): reduces load on ICE.gov when multiple clients / StructuredNewsLd hit the API. */
const CACHE_TTL_MS =
...
export async function GET() {
  ...
  try {
    ...
    // Return stale cache if available when upstream fails
    if (cache) {
      return NextResponse.json({
        items: cache.items,
        updated: cache.updated,
