...
import { summarizeInBrowser } from "@/app/lib/client-ai";
  ...
  const handleAiSummarize = async () => {
    ...
    try {
      ...
      const combinedText = items
        .map((n) => [n.title, n.description, n.state].filter(Boolean).join(" "))
        .join(" ");
      const summary = await summarizeInBrowser(combinedText);
      setAiSummary(summary);
      trackEvent(ANALYTICS_EVENTS.AI_SUMMARY_SUCCESS, { provider: "browser" });
    } catch (err) {
      const message = err instanceof Error ? err.message : "Summarization failed";
      setAiError(message);
      trackEvent(ANALYTICS_EVENTS.AI_SUMMARY_ERROR, { error: message });
    } finally {
      setAiLoading(false);
    }
  };
    ...
    <div className="h-screen flex flex-col">
      ...
      <header className="flex-shrink-0 border-b border-[#2a303c] bg-[#0c0e12]/95 backdrop-blur px-4 py-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs text-[#6b7280] hidden sm:inline">U.S. ICE · News, map, Know Your Rights, hotlines, in-browser AI</span>
        </div>
      ...
      <div className="flex-1 flex min-h-0">
        <aside className="w-full sm:w-[400px] flex-shrink-0 flex flex-col border-r border-[#2a303c] bg-[#0c0e12] overflow-hidden">
          <div className="p-3 border-b border-[#2a303c] space-y-2">
            ...
            {/* AI summary: in-browser (Transformers.js), no API keys */}
            <div className="pt-1 border-t border-[#2a303c]">
              ...
              >
                {aiLoading ? "Summarizing in browser…" : "Summarize with AI (in-browser)"}
              </button>
              <p className="text-[10px] text-[#6b7280] mt-0.5">Runs in your browser; first time may load a model.</p>
              ...
              {aiSummary && (
                <div className="mt-2 p-2 rounded border border-[#2a303c] bg-[#161a22] text-xs text-[#9ca3af] whitespace-pre-wrap">
                  <span className="text-[10px] text-[#a78bfa] uppercase mr-2">In-browser</span>
                  {aiSummary}
                </div>
              )}
            </div>
