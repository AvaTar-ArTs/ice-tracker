...
import KnowYourRights from "@/app/components/KnowYourRights";
import ResourcesHotlines from "@/app/components/ResourcesHotlines";
...
function HomeContent() {
  ...
  const [showKnowYourRights, setShowKnowYourRights] = useState(false);
  const [showResourcesHotlines, setShowResourcesHotlines] = useState(false);
  ...
  const handleAiSummarize = async () => {
    ...
    setAiSummary(null);
    setAiProvider(null);
    trackEvent(ANALYTICS_EVENTS.AI_SUMMARY_REQUEST);
    try {
      const payload = filtered.slice(0, 15).map((n) => ({
        title: n.title,
      ...
      const data = await res.json();
      if (!res.ok) {
        setAiError(data.hint ?? data.error ?? "Summarization failed");
        trackEvent(ANALYTICS_EVENTS.AI_SUMMARY_ERROR, { error: data.error });
        return;
      }
      setAiSummary(data.summary);
      setAiProvider(data.provider ?? null);
      trackEvent(ANALYTICS_EVENTS.AI_SUMMARY_SUCCESS, { provider: data.provider });
    } catch {
      setAiError("Network error");
      trackEvent(ANALYTICS_EVENTS.AI_SUMMARY_ERROR, { error: "network" });
    } finally {
      setAiLoading(false);
    }
  };
    ...
    <div className="h-screen flex flex-col">
      ...
      <header className="flex-shrink-0 border-b border-[#2a303c] bg-[#0c0e12]/95 backdrop-blur px-4 py-3 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <h1 className="text-lg font-semibold text-[#e2e6ec] tracking-tight">ICE Activity Tracker</h1>
          <span className="text-xs text-[#6b7280] hidden sm:inline">U.S. ICE · News, map, Know Your Rights, hotlines</span>
        </div>
        <div className="flex items-center gap-2 text-xs text-[#9ca3af]">
          ...
          {newCount > 0 && (
            <button type="button" onClick={() => setNewCount(0)} className="px-2 py-1 rounded bg-[#065f46] text-[#6ee7b7]">
              {newCount} new
            </button>
          )}
          <button
            type="button"
            onClick={() => { setLoading(true); fetchNews(); trackEvent(ANALYTICS_EVENTS.REFRESH_CLICK); }}
      ...
      <div className="flex-1 flex min-h-0">
        <aside className="w-full sm:w-[400px] flex-shrink-0 flex flex-col border-r border-[#2a303c] bg-[#0c0e12] overflow-hidden">
          <div className="p-3 border-b border-[#2a303c] space-y-2">
            {fetchError && (
              <div className="p-2 rounded bg-[#7f1d1d]/30 border border-[#991b1b] text-[#fca5a5] text-xs flex items-center justify-between gap-2">
                ...
                <button type="button" onClick={() => { setLoading(true); fetchNews(); }} className="px-2 py-1 rounded bg-[#991b1b] hover:bg-[#b91c1c]">Retry</button>
              </div>
            )}
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                type="button"
                onClick={() => { setShowReportForm((v) => !v); trackEvent(ANALYTICS_EVENTS.REPORT_FORM_OPEN); }}
            ...
            {/* In-app: Know Your Rights */}
            <div>
              <button
                type="button"
                onClick={() => { setShowKnowYourRights((v) => !v); setShowResourcesHotlines(false); trackEvent(ANALYTICS_EVENTS.KNOW_YOUR_RIGHTS_TOGGLE); }}
                className="text-xs text-[#93c5fd] hover:underline font-medium"
              >
                {showKnowYourRights ? "Hide Know Your Rights" : "Know Your Rights"}
              </button>
            </div>
            {showKnowYourRights && (
              <KnowYourRights isOpen onClose={() => setShowKnowYourRights(false)} />
            )}

            {/* In-app: Rapid response & hotlines */}
            <div>
              <button
                type="button"
                onClick={() => { setShowResourcesHotlines((v) => !v); setShowKnowYourRights(false); trackEvent(ANALYTICS_EVENTS.RESOURCES_HOTLINES_TOGGLE); }}
                className="text-xs text-[#93c5fd] hover:underline font-medium"
              >
                {showResourcesHotlines ? "Hide hotlines" : "Rapid response & hotlines"}
              </button>
            </div>
            {showResourcesHotlines && (
              <ResourcesHotlines isOpen selectedState={filterState} onClose={() => setShowResourcesHotlines(false)} />
            )}
            ...
            {showReportForm && (
              <form onSubmit={handleSubmitReport} className="p-3 rounded-lg border border-[#2a303c] bg-[#161a22] space-y-2">
                ...
                <p className="text-xs text-[#f59e0b]">If someone needs help now, open <strong>Rapid response & hotlines</strong> above for hotline numbers.</p>
                <input
            ...
            <button type="button" onClick={() => { setShowFaq((v) => !v); trackEvent(ANALYTICS_EVENTS.FAQ_TOGGLE); }} className="text-xs text-[#93c5fd] hover:underline">
              {showFaq ? "Hide FAQ" : "FAQ"}
            </button>
            <button type="button" onClick={() => { setShowLiveTips((v) => !v); trackEvent(ANALYTICS_EVENTS.LIVE_TIPS_TOGGLE); }} className="text-xs text-[#93c5fd] hover:underline">
              {showLiveTips ? "Hide" : "Ways to get live updates"}
            </button>

            {/* AI summary: only shown when NEXT_PUBLIC_AI_ENABLED=true (and you set a key). Omit or set false to go live without your own API. */}
            {process.env.NEXT_PUBLIC_AI_ENABLED === "true" && (
              <div className="pt-1 border-t border-[#2a303c]">
                <button
                  type="button"
                  onClick={handleAiSummarize}
                  disabled={aiLoading || filtered.length === 0}
                  className="text-xs text-[#a78bfa] hover:underline disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {aiLoading ? "Summarizing…" : "Summarize with AI (OpenAI/Gemini)"}
                </button>
                {aiError && <p className="text-[10px] text-[#f87171] mt-1">{aiError}</p>}
                {aiSummary && (
                  <div className="mt-2 p-2 rounded border border-[#2a303c] bg-[#161a22] text-xs text-[#9ca3af] whitespace-pre-wrap">
                    {aiProvider && <span className="text-[10px] text-[#a78bfa] uppercase mr-2">{aiProvider}</span>}
                    {aiSummary}
                  </div>
                )}
              </div>
            ...
            {/* In-app: Ways to get live updates (all content here, no external links required for core) */}
            {showLiveTips && (
              <div className="p-3 rounded-lg border border-[#2a303c] bg-[#161a22] text-xs text-[#9ca3af] space-y-2">
                <p className="font-medium text-[#e2e6ec]">Ways to get live updates (in this app and beyond)</p>
                <ul className="list-disc list-inside space-y-1">
                  <li><strong>This app</strong> — Use the refresh dropdown (1 min, 5 min, 15 min) to poll ICE.gov news more often. Open <strong>Know Your Rights</strong> and <strong>Rapid response & hotlines</strong> above for in-app content and state hotlines.</li>
                  <li><strong>Community report</strong> — Use “+ Report” to log what you see; data stays in this browser only.</li>
                  <li><strong>SMS / nationwide alerts</strong> — In <strong>Rapid response & hotlines</strong> you’ll find a national option to text REPORT for alerts; no app download.</li>
                  <li><strong>Official ICE</strong> — ICE.gov RSS (which this app uses) and @ICEgov on X/Twitter for press releases.</li>
                  <li><strong>Local hotlines</strong> — Open <strong>Rapid response & hotlines</strong> and pick your state filter to see state-specific rapid response numbers.</li>
                </ul>
              </div>
            )}
          </div>
          <div id="main-content" className="flex-1 overflow-y-auto p-2" role="feed" aria-label="Latest ICE enforcement and ERO news" tabIndex={-1}>
            <h2 id="feed-heading" className="text-xs font-medium text-[#9ca3af] px-1 mb-2">Latest ICE activity & enforcement news</h2>
                  ...
                  <li key={item.id}>
                    <article
                      className={`rounded-lg border p-3 cursor-pointer transition-colors ${
                        selectedId === item.id ? "border-[#3b82f6] bg-[#1e3a5f]/30" : "border-[#2a303c] bg-[#161a22] hover:bg-[#1f2430]"
                      }`}
                    ...
                    >
                      <a
                        ...
                        onClick={(e) => { e.stopPropagation(); trackEvent(ANALYTICS_EVENTS.NEWS_CLICK, { link: item.link, state: item.state ?? "" }); }}
                      >
                        {item.title}
                      </a>
      ...
      <footer className="flex-shrink-0 border-t border-[#2a303c] px-4 py-2 text-xs text-[#6b7280] flex flex-wrap items-center gap-x-4 gap-y-1">
        <span>Data: ICE.gov RSS. Know Your Rights & hotlines in-app. Not legal advice.</span>
